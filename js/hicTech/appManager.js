...
(function($) {
    $.jQTPanel = function($elem,_jQTSettings){
		...
		function init(){
		    ...
		    //addPageToHistory(_this.currentPage);    
    ...
    $.jQTouch = function(options) {
        ...
        function init() {
			...
			function backKeyDown(e) {
				console.log('back'); 
				liveTap({target:$(jQTSettings.backSelector)});
			}		    
            ...
            $(document).ready(function() {
				...
				current_page = (location.href.indexOf('#')>0) ? location.href.substring(location.href.lastIndexOf('#') + 1) : 0;

				if (current_page) {
					$('#' + current_page).addClass('current');
				}
				setHash(current_page);
